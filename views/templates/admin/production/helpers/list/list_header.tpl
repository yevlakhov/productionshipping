...
.bootstrap select[multiple].exclude_selected{
    min-height: 150px;
    min-width: 250px;
}
						...
						<th {if isset($params.align)} class="{$params.align}" {/if}>
							...
							{else}
								...
								{elseif $params.type == 'select'}
									...
									{/if}
                                {elseif $params.type == 'exclude_selected'}
									{if isset($params.filter_key)}
                                        {assign var="selected_statuses" value=$params.value|unserialize}
										<select class="exclude_selected" multiple class="filter" name="{$list_id}Filter_{$params.filter_key}[]" {if isset($params.width)} style="width:{$params.width}px"{/if}>
											<option value="" {if $params.value == ''} selected="selected" {/if}>-</option>
											{if isset($params.list) && is_array($params.list)}
												{foreach $params.list AS $option_value => $option_display}
													<option value="{$option_value}" {if (string)$option_display === (string)$params.value ||  $option_value|in_array:$selected_statuses} selected="selected"{/if}>{$option_display}</option>
												{/foreach}
											{/if}
										</select>
									{/if}
